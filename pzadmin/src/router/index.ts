...
const routes = [
  { 
    ...
    component: Layout,
    redirect: () => {
      // 持久化数据可能为空或损坏，任何异常都回退到控制台。
      try {
        const localData = localStorage.getItem('v3pz')
        const routerList = localData ? JSON.parse(localData).menu?.routerList : []
        const first = Array.isArray(routerList) ? routerList[0] : undefined
        return first?.children?.[0]?.meta?.path || first?.meta?.path || '/dashboard'
      } catch {
        return '/dashboard'
      }
    },
    ...
    children: [
      ...
      // {
      //   path: 'vppz',
      //   meta: { id: '3', name: '安护陪诊', icon: 'BellFilled' },
      //   children: [
      //     {
      //       path: '',
      //       alias: ['staff'],
