<template>
  ...
  <van-row justify="space-around">
    <van-col v-for="(item, index) in homeData.nav2s" :key="item.id" class="center-img" span="11"
...
<script setup lang="ts">
  ...
  const onSearch = () => {
    ...
    if (!keyword) return;
    const hospitals = homeData.value.hospitals || [];
    const match = hospitals.find((item: any) => String(item.name || '').toLowerCase().includes(keyword));
    if (match) goOrder(match);
    else showNotify({ type: 'warning', message: '未找到匹配的医院' });
  };
  const goOrder = (item: any) => {
    router.push(`/createOrder?id=${item.id}`);
  };
  const goOrderTwo = (index: any) => {
